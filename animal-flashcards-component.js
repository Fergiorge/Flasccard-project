...
import { Sparkles, ArrowRight, RotateCcw, Volume2, VolumeX, Trophy } from 'lucide-react';
...
const AnimalFlashcards = () => {
  ...
  const playAnimalSound = (animal) => {
    ...
    if (animal === 'turtle' || animal === 'tortoise') {
      const osc = audioContext.createOscillator();
      const gain = audioContext.createGain();
      osc.connect(gain);
      gain.connect(audioContext.destination);
    ...
    } else if (animal === 'rabbit' || animal === 'hare') {
      const osc = audioContext.createOscillator();
      const gain = audioContext.createGain();
      osc.connect(gain);
      gain.connect(audioContext.destination);
      ...
      osc.stop(now + 0.1);
    } else if (animal === 'butterfly' || animal === 'moth') {
      const osc = audioContext.createOscillator();
      const gain = audioContext.createGain();
      ...
      osc.connect(gain);
      gain.connect(audioContext.destination);
      osc.frequency.value = animal === 'butterfly' ? 1200 : 800;
  ...
  const handleFlip = () => {
    ...
    if (!isFlipped && selectedTheme === 'animals' && card.sound) {
      playAnimalSound(card.sound);
    } else if (!isFlipped && selectedTheme !== 'animals') {
      playSound('flip');
    }
